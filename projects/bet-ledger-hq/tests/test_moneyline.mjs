...
const nflMarginOnly={...nfl,details:[{game_id:'1',projection:{mu:-.64,ratings_known:true}}],board:[]};
assert.equal(C.cards('nfl',nflMarginOnly,now)[0].predicted,'away','NFL predicts from the model margin without prices, betting tiers or a win percentage, just like college');
assert.equal(C.cards('nfl',nflMarginOnly,now)[0].probability,null,'Do not invent a percentage when the model exports only a projected margin');
