...
try{
  ...
  for(const width of [320,393,1440]){
    ...
    await page.goto(origin+'/bet-ledger-hq/#moneyline');
    let frame=page.frameLocator('#frame-moneyline');
    ...
    await page.reload();
    frame=page.frameLocator('#frame-moneyline');
    await frame.locator('#status').filter({hasText:'Forecasts checked'}).waitFor();
    assert.equal(await frame.locator('.team-option[aria-pressed=true]').count(),3,'Selections survive reload');
    assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
    const child=page.frames().find(f=>f.url().endsWith('moneyline.html'));
    assert.ok(await child.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
    await frame.getByRole('button',{name:'Make card',exact:true}).click();
    for(const name of ['Gridiron Gold','Ballpark Stubs','Jumbotron','True North','Sportsbook Slip']){
      await frame.locator('.style-option',{hasText:name}).click();
      await frame.locator('#card-status').filter({hasText:/3 picks · ML-/}).waitFor();
      assert.ok(await child.evaluate(()=>{const c=document.querySelector('#card-canvas');return c.width===1080&&c.height>=1350;}),name+' card renders');
    }
    await frame.getByRole('button',{name:'Close card studio',exact:true}).click();
    await page.screenshot({path:'test-results/moneyline-'+width+'.png',fullPage:true});
  ...
  console.log('Moneyline browser: 320px, 393px, desktop; selection, persistence, filters, failures, locks and no overflow passed');
