(() => {
  ...
  function render(){
    ...
    $('#print').disabled=!picks.length;$('#make-card').disabled=!picks.length;
  ...
  /* ---------- card studio ---------- */
  const STYLE_KEY='kevbot-moneyline-card-style',SWATCH={gridiron:'linear-gradient(135deg,#ffd772,#8a5a12 55%,#111)',ballpark:'linear-gradient(135deg,#f6ecd6 50%,#0c1a2e 50%)',scoreboard:'radial-gradient(circle,#ffb000 30%,#050608 32%)',northern:'linear-gradient(90deg,#c8102e 28%,#fff 28% 72%,#c8102e 72%)',slip:'repeating-linear-gradient(0deg,#fff 0 4px,#ddd 4px 6px)'};
  let cardStyle='gridiron',drawing=0,lastMeta=null;
  try{const s=localStorage.getItem(STYLE_KEY);if(s&&window.MoneylineCard.styles[s])cardStyle=s;}catch(_){}
  function cardPicks(){
    const lookup=new Map(allCards().map(c=>[c.key,c]));
    return Object.values(saved).filter(r=>r.day===$('#date').value&&($('#sport').value==='all'||r.sport===$('#sport').value)).map(r=>{
      const c=lookup.get(r.key);let prob=r.win_prob??null;
      if(prob==null&&c?.probability!=null&&c.predicted)prob=r.side===c.predicted?c.probability:1-c.probability;
      return {...r,prob,teamName:r[r.side+'Name']||c?.[r.side+'Name']||r.team};
    });
  }
  async function drawCard(){
    const token=++drawing,picks=cardPicks();$('#card-status').textContent='Drawing '+window.MoneylineCard.styles[cardStyle].name+'…';
    document.querySelectorAll('.style-option').forEach(b=>b.setAttribute('aria-checked',String(b.dataset.style===cardStyle)));
    try{const out=await window.MoneylineCard.draw($('#card-canvas'),picks,{style:cardStyle,day:$('#date').value});if(token!==drawing)return;lastMeta=out.meta;
      $('#card-status').textContent=picks.length+' pick'+(picks.length===1?'':'s')+' · '+out.meta.serial+(picks.some(p=>p.prob==null)?' · Some games have no model win % (shown as —)':'');}
    catch(e){if(token===drawing)$('#card-status').textContent='Could not draw this card. Try another style.';}
  }
  const blob=()=>new Promise(r=>$('#card-canvas').toBlob(r,'image/png'));
  const fileName=()=>'kevbot-moneyline-'+$('#date').value+'-'+cardStyle+'.png';
  $('#card-styles').innerHTML=Object.entries(window.MoneylineCard.styles).map(([k,v])=>'<button type="button" role="radio" class="style-option" data-style="'+k+'" aria-checked="false"><i style="background:'+SWATCH[k]+'"></i><span><b>'+esc(v.name)+'</b><small>'+esc(v.note)+'</small></span></button>').join('');
  $('#card-styles').addEventListener('click',e=>{const b=e.target.closest('[data-style]');if(!b)return;cardStyle=b.dataset.style;try{localStorage.setItem(STYLE_KEY,cardStyle);}catch(_){}drawCard();});
  $('#make-card').addEventListener('click',()=>{const d=$('#card-dialog');if(typeof d.showModal==='function')d.showModal();else d.setAttribute('open','');$('#card-share').hidden=!(navigator.canShare&&navigator.canShare({files:[new File([''],'x.png',{type:'image/png'})]}));drawCard();});
  $('#card-close').addEventListener('click',()=>$('#card-dialog').close?.());
  $('#card-download').addEventListener('click',async()=>{const b=await blob();if(!b)return;const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=fileName();document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),4000);});
