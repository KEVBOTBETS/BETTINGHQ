...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;if(root)root.Moneyline=api;})(typeof self!=='undefined'?self:this,function(){
  'use strict';
  ...
  function choose(saved,card,side,now=Date.now()){
    ...
    return {...saved,[card.key]:{key:card.key,sport:card.sport,start:card.start,day:card.day,
      home:card.home,away:card.away,homeName:card.homeName,awayName:card.awayName,side,team:card[side],predicted:card.predicted?card[card.predicted]:null,
      win_prob:card.probability==null||!card.predicted?null:(side===card.predicted?card.probability:1-card.probability),
