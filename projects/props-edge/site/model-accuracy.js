...
(function (root) {
  ...
  function render(host, data) {
    ...
    const projectionOnly=Boolean(data.projection_only);
    const props=Object.entries(data.props||{});
    const scope=data.scope||{};
    const history=data.history||{};
    const scopeText=[scope.season,scope.season_type_label].filter(Boolean).join(" ");
    const winnerLosses=Math.max(0,(g.winner?.n||0)-(g.winner?.correct||0));
    ...
    host.innerHTML=`<style>.model-accuracy{color:inherit;margin:18px 0}.model-accuracy p{line-height:1.6}.model-accuracy h3{margin:22px 0 10px}.ma-grid{display:flex;gap:12px;flex-wrap:wrap}.ma-stat{padding:12px 16px;border:1px solid #8885;border-radius:8px;min-width:130px}.ma-stat b{display:block;font-size:23px}.ma-scroll{overflow:auto;max-height:580px;border:1px solid #8885;border-radius:8px}.model-accuracy table{border-collapse:collapse;width:100%;font-size:13px;min-width:680px}.model-accuracy th,.model-accuracy td{text-align:left;padding:10px;border-bottom:1px solid #8883;white-space:normal}.model-accuracy th{font-weight:700}.ma-controls{display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin:12px 0}.model-accuracy select,.model-accuracy input,.model-accuracy button{font:inherit;color:inherit;background:transparent;border:1px solid #8888;border-radius:5px;padding:7px}.model-accuracy option{color:#222;background:#fff}.model-accuracy small{opacity:.75}.ma-ok{color:#299c69}.ma-loss{color:#d45c55}</style>
      ...
      ${scopeText?`<p class="ma-scope"><b>${esc(scopeText)} only</b> · historical seasons and preseason are excluded from every number below.</p>`:""}
      <p><small>Updated ${esc(data.generated_at)} · <a href="data/accuracy.json" download>Download accuracy snapshot</a>. Only frozen pregame forecasts from the displayed season are counted.${history.truncated?` Season totals use all ${history.total} forecasts; the table and download include the latest ${history.displayed}.`:""}</small></p>
      ${g.logged ? `<h3>Every game forecast, with or without a bet</h3><div class="ma-grid"><div class="ma-stat">Game winner record<b>${g.winner?.correct||0}–${winnerLosses}</b>${pct(g.winner?.accuracy)} · ${g.winner?.n||0} decided</div><div class="ma-stat">Games tracked<b>${g.graded||0} graded</b>${g.pending||0} pending / ${g.logged} logged</div><div class="ma-stat">Spread direction<b>${pct(g.ats?.accuracy)}</b>${g.ats?.n||0} decided</div><div class="ma-stat">Total direction<b>${pct(g.totals?.accuracy)}</b>${g.totals?.n||0} decided</div><div class="ma-stat">Margin / total MAE<b>${num(g.margin_mae,1)} / ${num(g.total_mae,1)}</b>Points of absolute error</div><div class="ma-stat">Margin: model / market<b>${num(g.margin_comparison?.model_mae,1)} / ${num(g.margin_comparison?.market_mae,1)}</b>${g.margin_comparison?.n||0} matched games</div><div class="ma-stat">Total: model / market<b>${num(g.total_comparison?.model_mae,1)} / ${num(g.total_comparison?.market_mae,1)}</b>${g.total_comparison?.n||0} matched games</div></div>`:""}
      ${projectionOnly?"":`<h3>Preferred market picks</h3><div class="ma-grid"><div class="ma-stat">Picks saved<b>${o.logged||0}</b>${o.pending||0} pending</div><div class="ma-stat">Market-pick record<b>${o.wins||0}–${o.losses||0}–${o.pushes||0}</b>${o.voids||0} void · ${pct(o.win_rate)} wins</div><div class="ma-stat">Flat one-unit result<b>${num(o.units)}</b>${pct(o.roi)} hypothetical ROI</div><div class="ma-stat">Pick Brier<b>${num(o.brier,3)}</b>All selected tiers</div></div>`}
      ...
      ${projectionOnly?"":`${groupTable("Accuracy by original tier",data.by_tier)}${groupTable("Accuracy by market",data.by_market)}<details><summary>Probability calibration and tier versions</summary>${groupTable("Tier calculation version",data.by_version)}<div class="ma-scroll"><table><thead><tr><th>Probability bucket</th><th>Sample</th><th>Mean prediction</th><th>Actual win rate</th></tr></thead><tbody>${(data.calibration||[]).map(c=>`<tr><td>${esc(c.bucket)}</td><td>${c.n}</td><td>${pct(c.predicted)}</td><td>${pct(c.actual)}</td></tr>`).join("")||'<tr><td colspan="4">Waiting for decided picks.</td></tr>'}</tbody></table></div></details>`}
      <h3>${projectionOnly?"Player projection history":"Prediction history"}</h3><p>${projectionOnly?"Frozen player forecasts are graded from final box scores. No wager, price, stake, or ledger history is stored here.":"Game forecasts and preferred market picks are shown by default. The complete audit also includes the opposite priced sides; its combined win rate is not a measure of pick quality."}</p>
      <div class="ma-controls"><label>Show <select data-ma-kind>${projectionOnly?'<option value="prop">Player forecasts</option>':'<option value="preferred">Forecasts + preferred picks</option><option value="all">Complete priced-side audit</option><option value="game">Game forecasts</option><option value="prop">Player forecasts</option><option value="call">Preferred market picks</option>'}</select></label><label>Result <select data-ma-result><option value="all">All</option>${(projectionOnly?["Pending","Void","Graded"]:["Pending","Win","Loss","Push","Void","Graded"]).map(x=>`<option>${x}</option>`).join("")}</select></label>${projectionOnly?"":`<label>Tier <select data-ma-tier><option value="all">All</option>${[...new Set(rows.map(r=>r.tier).filter(Boolean))].sort().map(x=>`<option>${esc(x)}</option>`).join("")}</select></label>`}<label>Search <input data-ma-search placeholder="Player, market or date" type="search"></label></div>
    ...
    function draw(){
      const kind=q("[data-ma-kind]").value, result=q("[data-ma-result]").value, tier=q("[data-ma-tier]")?.value||"all", search=q("[data-ma-search]").value.toLowerCase();
