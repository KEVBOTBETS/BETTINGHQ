...
function boardInPage() {
  ...
  return { ctx, adapter, L: ctx.window.NFLEdgeLedger, BS: ctx.window.BetSync,
           put: rows => localStorage.setItem(ctx.window.NFLEdgeLedger.STORAGE_KEY, JSON.stringify({ entries: rows })),
           get: () => ctx.window.NFLEdgeLedger.load() };
}

const entry = (over = {}) => ({
  id: "401872659|TOTAL|over", candidate_id: "401872659:TOTAL:over",
  game_id: "401872659", game_date: "2026-09-13", week: 2, season_type: "REG",
  matchup: "Baltimore @ Indianapolis", market: "TOTAL", side: "over",
  pick: "Over 46.5", line: 46.5, price: -108, book: "ESPN BET",
  model_prob: 0.55, market_fair_prob: 0.519, breakeven: 0.519,
  edge: 0.031, edge_raw: 0.055, tier: "LEAN", confidence: 0.62,
  stake: 6.75, added_at: "2026-09-13T14:00:00.000Z",
  result: null, pnl: null, final_score: null,
  ...over
});
...
test("a bet flattens into the sheet's columns without losing itself", () => {
  ...
  assert.equal(rows[0].app, "nfl-lab");
  assert.ok(rows[0].id.startsWith("nfl-lab:"), "ids carry the board, so two boards never collide");
...
test("a result settled on another device lands on this board's entry", () => {
  ...
  assert.equal(after.pnl, 7.5);
  assert.equal(after.stake, 6.75, "settling must not disturb the stake");
});
...
test("a stake edited elsewhere lands too, without disturbing the rest", () => {
  ...
  assert.equal(after.game_id, "401872659");
  assert.equal(after.pick, "Over 46.5");
  assert.equal(after.line, 46.5);
});
...
test("the shared bankroll reaches the staking settings", () => {
  const b = boardInPage();
  b.adapter.applyBankroll({ current: 407.15 });
  assert.equal(b.ctx.window.EdgeStaking.load("nfl-edge.staking.v1").bankroll, 407.15);
});
