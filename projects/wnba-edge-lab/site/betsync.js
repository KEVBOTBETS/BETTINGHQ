...
(function (root, factory) {
  ...
  function request(cfg, payload) {
    ...
    return postJson(cfg.url, body).catch(function (err) {
      if (body.action !== "pull" && body.action !== "audit") throw err;
      return jsonp(cfg.url, { token: cfg.token, action: body.action, since: body.since || "", before: body.before || "", limit: body.limit || 25 });
    }).then(function (res) {
  ...
  function auditHistory(cfg, before) {
    return request(cfg || loadConfig(), {action:"audit", before:before || "", limit:25});
  }
  function undoChange(cfg, id, revision) {
    var c=cfg || loadConfig();
    return request(c,{action:"undo",audit_id:id,expected_rev:revision,device:c.device,app:"hq"});
  }
  ...
  function friendlyError(err) {
    ...
    if (msg === "undo_conflict") return "This record changed after the history was loaded. Refresh and review the latest change before undoing.";
  ...
  var api = {
    ...
    pullAll: pullAll, pushRows: pushRows, auditHistory: auditHistory, undoChange: undoChange, friendlyError: friendlyError
  };
  return api;
});
