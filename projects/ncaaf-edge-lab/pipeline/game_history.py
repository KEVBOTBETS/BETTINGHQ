...
def update(state_path, output_path, candidates, forecast_games, all_games, league,
    ...
    # authoritative schedule before applying the current-season-only boundary.
    ...
    if game_only:
        log = {
            "game_only": True,
            "records": {
                key: row for key, row in log.get("records", {}).items()
                if row.get("kind") == "game"
                and (season is None or str(row.get("season")) == str(season))
                and (season_type is None or str(row.get("season_type")) == str(season_type))
            },
        }
    else:
