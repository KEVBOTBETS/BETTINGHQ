...
def open_bet(ledger: dict, cand: dict, bankroll: float, cfg: dict) -> bool:
    ...
    stake = M.stake_for(cand["model_prob"], cand["price"], bankroll, cfg,
                        edge=cand.get("action_edge", cand.get("edge")), push_prob=cand.get("push_prob", 0))
    risk_mult = min(1.0, max(0.0, float(cand.get("stake_multiplier", 1.0))))
    step = float(cfg["bankroll"].get("round_stake_to") or 0.5)
    stake = round((stake * risk_mult) / step) * step if step > 0 else stake * risk_mult
    stake = round(stake, 2)
    if stake <= 0:
        return False
    ledger[key] = {
...
def _attach_clv(bet: dict, lines: dict) -> None:
    ...
    if market == "ATS" and cl is not None and bet.get("line") is not None:
        ...
        clv += gained * 0.025   # ~2.5 pts of win probability per point of spread
